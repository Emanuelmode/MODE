...
class RegimeDetector:
    ...
    def classify(self, lam: float, lz: float = None, d2: float = None) -> str:
        ...
        Fix 3: λ (Rosenstein puro Python) subestima sistemáticamente
        en series cortas. D₂ y LZ son más estables como criterio primario.

        Prioridad: λ<0 → estable (confiable)
                   luego D₂ + LZ como árbitros
        """
        # λ negativo es diagnóstico confiable de sistema estable
        if not np.isnan(lam) and lam < 0:
            return 'stable'

        # Clasificación multi-métrica con D₂ y LZ
        if d2 is not None and not np.isnan(d2) and \
           lz is not None and not np.isnan(lz):

            # LZ > 0.95 con D2 alto → ruido puro, no caos estructurado
            if lz > 0.95 and d2 > 2.3:
                return 'noisy'
            if lz > 0.75 and d2 > 1.9:
                return 'hyperchaotic'
            if lz > 0.50 and d2 > 1.5:
                return 'chaotic'
            if lz < 0.25 and d2 < 1.5:
                return 'stable'
            return 'weakly_chaotic'

        # Fallback a LZ solo
        if lz is not None and not np.isnan(lz):
            if lz < 0.3:   return 'stable'
            if lz < 0.6:   return 'weakly_chaotic'
