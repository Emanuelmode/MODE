...
try:
    ...
    # Fallback defensivo si el archivo no existe
...
class SemidynamicTau:
    ...
    def compute(self, x: np.ndarray, regime: str = 'unknown') -> int:
        if regime in self._cache: return self._cache[regime]
...
class Metrics:
    ...
    def lyapunov(x: np.ndarray, tau: int, m: int = 3, min_tsep: int = None, max_iter: int = 300) -> float:
        Y = embed(x, m, tau); N = len(Y)
        if min_tsep is None: min_tsep = max(1, int(0.1 * N))
        dists = cdist(Y, Y); np.fill_diagonal(dists, np.inf)
        divergences = []
        for i in range(N):
    ...
    def correlation_dimension(Y: np.ndarray, n_r: int = 20) -> float:
        ...
        dists = cdist(Y, Y); flat = dists[np.triu_indices(N, k=1)]
        flat = flat[flat > 0]
    ...
    def lempel_ziv(x: np.ndarray, Y: np.ndarray = None) -> float:
        seq = (Y - Y.mean(axis=0)) @ np.linalg.eigh(np.cov(Y.T))[1][:, -1] if Y is not None and Y.shape[0] > 10 else x
        binary = ''.join('1' if v > np.median(seq) else '0' for v in seq)
        n, c, l, i, k, k_max, stop = len(binary), 1, 1, 0, 1, 1, False
        while not stop:
        ...
        return float(np.clip(c / (n / (np.log2(n) + 1e-10)), 0, 2))
...
class DeltaLibrary:
    ...
    def get(self, regime: str) -> float:
        if DELTA_LIBRARY and regime in DELTA_LIBRARY:
            # Retorna un escalar representativo (media) para compatibilidad con UI
            vals = list(DELTA_LIBRARY[regime].values())
            return float(np.mean(vals))
        return self.FALLBACK_SCALARS.get(regime, 0.10)
    ...
    def score(self, x: np.ndarray, tau: int, m: int = 3) -> dict:
        ...
        for k, g in grads.items():
            if not np.isnan(g):
                ...
                stability_map[k] = {'gradient': g, 'stable': g < delta, 'delta': delta, 'weight': weight, 'w_compat': w_compat}
        ...
        coherent = (not np.isnan(r3_score) and r3_score >= self.COHERENCE_THRESHOLD and 
                    r3_min >= self.COHERENCE_THRESHOLD / 2 and regime != 'noisy')
        ...
        return {
            ...
            'delta': delta, 'metrics': metrics, 'gradients': grads, 'stability_map': stability_map, 'n_valid': valid_n
        }
