...
REVISIÓN 2026-05c: Fix indentación + typos + R3 dict-to-scalar
═══════════════════════════════════════════════════════════════
"""
import numpy as np
...
class DeltaLibrary:
    TABLE = {
        'stable': 0.06, 'weakly_chaotic': 0.07, 'chaotic': 0.08,
        'hyperchaotic': 0.15, 'noisy': 0.20,
    }
...
class AttractorPipeline:
    ...
    def run(self, x: np.ndarray, label: str = 'serie') -> dict:
        ...
        self._log(f" R³ score: {r3['R3_score']:.6f} | Coherente: {r3['coherent']}")
        self._log(f"{'═'*52}\n")
