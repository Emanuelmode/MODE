...
# ── Lector .atr MIT-BIH (anotaciones latido por latido) ─────────
def read_atr_bytes(atr_file, fs: int = 360) -> pd.DataFrame:
    """
    Decodifica archivo .atr MIT-BIH (formato binario propio).
    Devuelve DataFrame con columnas:
      sample   — índice de muestra del latido
      time_s   — tiempo en segundos
      beat_type — tipo de latido (N, V, A, /, etc.)
      description — descripción clínica del tipo
    """
    BEAT_CODES = {
        'N': 'Normal',
        'L': 'Bloqueo rama izquierda',
        'R': 'Bloqueo rama derecha',
        'B': 'Bundle branch block',
        'A': 'Latido auricular prematuro',
        'a': 'Auricular aberrado',
        'J': 'Juncional prematuro',
        'S': 'Supraventricular prematuro',
        'V': 'Ventricular prematuro (PVC)',
        'r': 'R-on-T ventricular',
        'F': 'Fusión ventricular/normal',
        'e': 'Latido auricular escape',
        'j': 'Latido juncional escape',
        'n': 'Supraventricular escape',
        'E': 'Ventricular escape',
        '/': 'Marcapasos',
        'f': 'Fusión marcapasos/normal',
        'Q': 'No clasificado',
        '?': 'No clasificado',
        '+': 'Cambio de ritmo',
        '~': 'Cambio de calidad de señal',
        '|': 'Artefacto aislado',
        's': 'ST change',
        'T': 'Onda T change',
        '*': 'Systole',
    ...
    while i + 1 < len(data):
        ...
        if anntype == 59:   # SKIP
            i += 2
            if i + 3 < len(data):
                skip = ((data[i] | (data[i+1] << 8)) << 16) | (data[i+2] | (data[i+3] << 8))
                sample += skip
                i += 4
            continue
        elif anntype == 60:  # NUM
            ...
            i += 2
            n_aux = data[i] if i < len(data) else 0
            i += 1 + n_aux + (n_aux % 2)
            continue
        elif anntype == 0 and diff == 0:  # EOF
            break

        sample += diff

        # Mapear anntype a carácter de anotación (tabla MIT-BIH)
        TYPE_MAP = {
            1:'N', 2:'L', 3:'R', 4:'a', 5:'V', 6:'F', 7:'J', 8:'A',
            9:'S', 10:'E', 11:'j', 12:'/', 13:'Q', 14:'~', 16:'|',
            17:'s', 18:'T', 19:'*', 20:'D', 21:'"', 22:'=', 23:'p',
            24:'B', 25:'n', 26:'e', 27:'+', 28:'r', 30:'^', 31:'|',
            32:'~', 34:'f', 35:'U', 38:'`', 41:'{', 42:'}',
        }
        beat_char = TYPE_MAP.get(anntype, '?')
        time_s    = round(sample / fs, 4)
        desc      = BEAT_CODES.get(beat_char, f'Tipo {anntype}')
    ...
    # Estadísticas RR
    if len(df) > 1:
        df['RR_prev_ms'] = np.concatenate([[np.nan],
                           np.diff(df['sample'].values) / fs * 1000])
    else:
        df['RR_prev_ms'] = np.nan
    return df


def summarize_atr(df_atr: pd.DataFrame) -> pd.DataFrame:
    """Resumen estadístico de las anotaciones."""
    if df_atr.empty:
        return pd.DataFrame()
    counts = df_atr['beat_type'].value_counts().reset_index()
    counts.columns = ['Tipo', 'N']
    counts['%'] = (counts['N'] / len(df_atr) * 100).round(2)
    desc_map = df_atr.drop_duplicates('beat_type').set_index('beat_type')['description']
    counts['Descripción'] = counts['Tipo'].map(desc_map)
...
def main():
    ...
    with st.sidebar:
        ...
        if modo == "Señal sintética":
            ...
            st.caption("Subir .dat + .hea del mismo registro. .atr opcional (anotaciones).")
            ...
            if "ecg_atr_df" not in st.session_state:
                st.session_state.ecg_atr_df = None

            updat = st.file_uploader("Archivo .dat", type=["dat","bin"], key="ecg_dat_file")
            uphea = st.file_uploader("Archivo .hea", type=["hea","txt"], key="ecg_hea_file")
            upatr = st.file_uploader("Archivo .atr (opcional — anotaciones)", type=["atr"], key="ecg_atr_file")
            ...
            # Leer .atr si se subió
            if upatr is not None and st.session_state.ecg_atr_df is None:
                try:
                    fs_atr = st.session_state.ecg_fsecg or 360
                    df_atr = read_atr_bytes(upatr, fs=fs_atr)
                    st.session_state.ecg_atr_df = df_atr
                    n_beats = len(df_atr)
                    n_pvc   = (df_atr['beat_type'] == 'V').sum() if not df_atr.empty else 0
                    st.success(f"Anotaciones cargadas: {n_beats} latidos · {n_pvc} PVC detectados")
                except Exception as e:
                    st.error(f"Error leyendo .atr: {e}")
            ...
            if st.session_state.ecg_sigfull is not None:
                ...
                end_idx   = min(int((ecgstart + ecgdur) * st.session_state.ecg_fsecg),
                                len(st.session_state.ecg_sigfull))
                if start_idx < len(st.session_state.ecg_sigfull):
                    ...
                    extra = {"fs": st.session_state.ecg_fsecg,
                             "start_s": ecgstart, "end_s": ecgstart+ecgdur}
            else:
    ...
    with tab3:
        ...
        # ── reference_library: alertas de rangos esperados ──────────
        st.divider()
        st.subheader("🔬 Validación contra literatura (reference_library)")
        range_check = r3.get("range_check", {})
        ref_ver     = r3.get("ref_version", "N/A")
        st.caption(f"reference_library v{ref_ver} · "
                   f"Rangos esperados para régimen **{r3['regime']}** "
                   f"según literatura clásica + calibración Duarte 2026")

        if range_check:
            fuera = {k: v for k, v in range_check.items() if not v.get("ok", True)}
            dentro = {k: v for k, v in range_check.items() if v.get("ok", True)}

            if fuera:
                st.warning(f"⚠ **{len(fuera)} métrica(s) fuera del rango esperado** "
                           f"para régimen '{r3['regime']}':")
                alert_rows = []
                for k, v in fuera.items():
                    alert_rows.append([
                        key_labels.get(k, k),
                        fmt(v.get("value"), 6) if v.get("value") is not None else "N/A",
                        v.get("msg", "---"),
                        "⚠ FUERA"
                    ])
                df_alert = pd.DataFrame(alert_rows,
                    columns=["Métrica", "Valor", "Detalle", "Estado"]).astype(str)
                st.dataframe(df_alert, use_container_width=True, hide_index=True)
            
            if dentro:
                ok_rows = []
                for k, v in dentro.items():
                    ok_rows.append([
                        key_labels.get(k, k),
                        fmt(v.get("value"), 6) if v.get("value") is not None else "N/A",
                ...
                df_ok = pd.DataFrame(ok_rows,
                    columns=["Métrica", "Valor", "Estado"]).astype(str)
                st.dataframe(df_ok, use_container_width=True, hide_index=True)

            if not fuera:
                st.success(f"✔ Todas las métricas dentro del rango esperado "
                           f"para régimen **{r3['regime']}**")
